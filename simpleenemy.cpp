...
std::vector<Bullet*> SimpleEnemy::getBullet(float nowTime) {
    ...
    if ((int)(dtime * 0.2) - (int)(dutime * 0.2) >= 1) {
        ret.push_back(new eldenBullet(nowTime, 12.0, (Vector2){x,y}, 100, 3, getEnemyBullets(), 0.5, getPlayer()->getPosition() - (Vector2){x,y}));
    }
    ...
    if ((int)(dtime * 0.8) - (int)(dutime * 0.8) >= 1) {
        for (int i = 0; i <= 20; i++) {
            ...
            Bullet *b = new basicBullet(nowTime, 100.0, (Vector2){x, y} + 10.0f * f, speed * f);
            ret.push_back(b);
        }
